...
void testMomentsEFIE(const Mesh::Triangle& obsTri, const Mesh::Triangle& srcTri) {
    ...
    size_t iPair = glPairsToIdx.at(std::minmax(obsTri.getIdx(), srcTri.getIdx()));
    ...
    for (const auto& [obs, obsWeight] : obsTri.getQuads()) {
        for (const auto& [src, srcWeight] : srcTri.getQuads()) {
            ...
            if (glTriPairs.nCommons[iPair] >= 2)
...
void testMomentsMFIE(const Mesh::Triangle& obsTri, const Mesh::Triangle& srcTri) {
    ...
    for (const auto& [obs, obsWeight] : obsTri.getQuads()) {
        for (const auto& [src, srcWeight] : srcTri.getQuads()) {
            ...
            else if (glTriPairs.nCommons[iPair] < 2)
    ...
    const auto& [m000, m001, m10, m01, m11] = 
        (obsTri.getIdx() <= srcTri.getIdx()) ? glTriPairs.momentsMFIE[iPair] : glTriPairs.momentsMFIE2[iPair];
    cmplx radMoments = 
...
void testSingularEFIE(const Mesh::Triangle& obsTri, const Mesh::Triangle& srcTri, std::ofstream& os) {
    ...
    std::cout << std::setprecision(15)
        << "# Common verts: " << glTriPairs.nCommons[iPair] << '\n'
...
void testSelfIntegrated(const Mesh::Triangle& obsTri, const Mesh::Triangle& srcTri) {
    ...
    double pairRad = obsTri.getSingularEFIE(srcTri, vobs, vsrc, iPair);
...
int main() {
    ...
    // auto srcs = Mesh::importMesh("config/rwg/test/n"+std::to_string(config.nsrcs)+"adj");
    ...
    /*
    for (const auto& [iTris, iPair] : Mesh::glPairsToIdx) {
        const auto& [obsTri, srcTri] = Mesh::glTriPairs.getTriPair(iTris);
    ...
    */
    ...
    auto srcTri = dynamic_pointer_cast<Mesh::RWG>(srcs[100])->getTris()[1];
    testSingularMFIE(obsTri, srcTri, file);
    testSingularMFIE(srcTri, obsTri, file);
