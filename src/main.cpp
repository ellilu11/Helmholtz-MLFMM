...
int main() {
    ...
    auto solver = std::make_unique<GMRES>(srcs, nf, root, EPS, config.maxIter);
    solver->solve("sol.txt");

    Time duration_ms0 = Clock::now() - start0;
    std::cout << " FMM total elapsed time: " << duration_ms0.count() << " ms\n\n";

    Mesh::printScattered(srcs, "ff_n"+to_string(nsrcs)+"_"+ieStr+".txt", 200);
    ...
    Mesh::printScattered(srcs, "ffDir_n"+to_string(nsrcs)+"_"+ieStr+".txt", 200);
